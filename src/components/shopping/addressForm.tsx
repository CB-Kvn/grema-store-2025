...
const AddressForm: React.FC<AddressFormProps> = ({
  ...
  useEffect(() => {
    console.log(availableDistritos);
  },[availableDistritos]) 
  ...
  useEffect(() => {
    if (selectedProvincia && selectedProvincia in cantones) {
      ...
      if (!cantones[selectedProvincia as ProvinciaId].find(c => c.nombre === values.canton)) {
        onChange({ ...values, canton: '', distrito: '', zipCode: '' });
      }
    }
  }, [selectedProvincia]);
  ...
  const handleProvinciaChange = (value: string) => {
    ...
    onChange({ ...values, provincia: value, canton: '', distrito: '', zipCode: '' });
  };
  ...
  const handleDistritoChange = (value: string) => {
    onChange({ ...values, distrito: value });
  };
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          <Label htmlFor={`${type}Distrito`}>Distrito</Label>
          <Select
            ...
            required
          >
            <SelectTrigger id={`${type}Distrito`}>
              <SelectValue placeholder="Seleccionar distrito" />
            </SelectTrigger>
            <SelectContent>
              {availableDistritos.map(distrito => (
                <SelectItem key={distrito.nombre} value={distrito.nombre}>
                  {distrito.nombre}
                </SelectItem>
              ))}
            </SelectContent>
